...
@router.put("/password")
def change_admin_password(payload: schemas.PasswordChange, db: Session = Depends(get_db), current_admin: models.AdminUser = Depends(auth.get_current_admin)):
    if not auth.verify_password(payload.current_password, current_admin.hashed_password):
        raise HTTPException(status_code=400, detail="La contraseña actual es incorrecta")
    
    current_admin.hashed_password = auth.get_password_hash(payload.new_password)
    db.commit()
